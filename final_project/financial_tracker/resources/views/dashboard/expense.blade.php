...
<form class="form-virtical ">
    <div class="form-group text-center">
        <label class="control-label col-xs-4 text-info" for="">Num of Expenses</label>
        ...
        <a href="{{config('app.url')}}/edit/transaction/{{$expense->id}}">
            <div class="col-xs-12 transaction_card_big">
                <div class="card-counter primary">
                    <div class="col-xs-5">
                        <span class="col-xs-12 count-name"><b>Title:</b> {{$expense->title}}</span>
                        <span class="col-xs-12 count-name"><b>Category:</b> {{$expense->category->title}}</span>                        
                        <span class="col-xs-12 count-name"><b>Date:</b> {{$expense->start_date}}</span>
                    </div>
                    <div class="col-xs-3">
                        <span class="col-xs-12 logo-big {{$expense->category->logo->class_name}}"></span>
                    </div>
                    <div class="col-xs-4">
                        <span class="col-xs-12 count-numbers">{{$expense->currency->code}} <strong>{{$expense->amount}}</strong></span>
                        <span class="col-xs-12 count-numbers"> % {{$expense->percentage}}</span>
                    </div>
                </div>
            </div>
        </a>
